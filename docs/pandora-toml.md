...
| `shards` | `strategy` (`argv` or `env`), `template` (`--shard={i}/{n}`), `env` (required with `strategy = "env"`), `default`, `max`, and for tier 2 `plan`, `expect_flag`, `report`, `plan_outputs`. `report` and `plan_outputs` may also carry `{argN}`. Every shard also gets `PANDORA_SHARD_INDEX` and `PANDORA_SHARD_TOTAL`. Remote only. |
