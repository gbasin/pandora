...
Eichler schedules local validation with Pueue: four machine-wide groups, a
...
deletion PR removes. The local executor is built and proved; the eichler side is
a feature-flagged seam, and nothing is deleted yet.

## What Pueue does today

Every line verified in `/Users/garybasin/Code/eichler-wt/journey-runner-proxy`
on `agent/journey-runner-proxy`, not assumed.

| # | Responsibility | Where | Who consumes it |
|---|---|---|---|
| 1 | Four machine-wide groups: `eichler-light` 2, `eichler-heavy` 1, `eichler-surfaces` 2, `eichler-stacks` 1 | `tools/validation/queue.mjs:50-70` (`setup`), limits re-asserted by `tools/stack/stack.mjs:47-52` | `plan.mjs` picks the group per suite; agents read the limits in `local-validation.md:19-31` |
| 2 | Which group a suite lands in | `tools/validation/plan.mjs:37` (default `eichler-heavy`), `:75` surfaces, `:249,255,280,291,301,314` light | the queue; nothing else |
...
| 6 | Receipts under `~/.local/state/eichler-validation/jobs/<uuid>/` — `request.json`, `running.json`, `cancel.json`, `result.json`, `recovery.json`, `cleanup-required` | written `queue.mjs:127,132,177,197,227`, `run.mjs:57,176`; read `state.mjs:56-58` | `pnpm validate result/status/cancel/recover`; `heavy.mjs` and `tools/browser-integration/run.mjs:188-229` write `cleanup-required` into it via `EICHLER_VALIDATION_DIRECTORY` |
...
| 11 | Nested-validation refusal | `EICHLER_VALIDATION_ACTIVE` set at `run.mjs:85`, refused at `validate.mjs` | a queued job that calls `pnpm validate` again |
...
* **No receipt reader outside `tools/validation/`.** `tools/proj.mjs` (82 KB)
  has zero matches for `pueue`, `validation` or `eichler-validation`. The
...
* **The only out-of-tree consumers are cleanup markers**, not receipts:
  ...
  into `EICHLER_VALIDATION_DIRECTORY`. That variable has to survive the
...
|---|---|---|
| 1 Groups with fixed slots | `[local] budget_mib` + `engine.admission`: memory held against a host budget, learned per (repo, job) | A group of N slots is a guess about size that is wrong for every member of the group. `eichler-light` held two jobs whether they were a 130 MiB `node --test` or a Turbo typecheck. |
| 2 Suite → group | `[[jobs]] size` in `pandora.toml` | The ceiling is declared; the reservation is learned from observed peaks. |
| 3 Worker count | Unchanged — eichler's planner still sets it | `PANDORA_CPUS` is a hint beside it. |
...
| 9 `GITHUB_ACTIONS` direct | Kept, generalised to `EICHLER_QUEUE=direct` | CI is unchanged. The local executor uses the same door. |
...
| `pnpm test:unit --project @eichler/domain x.test.ts` | same | none |
...
1. **Now (this branch).** `EICHLER_QUEUE=pandora|direct|pueue` in
...
2. **`pandora.toml` into the eichler repository root**, with the local jobs
...
3. **Flip the default** to `pandora` once the per-suite cancel grace (row 12)
   ...
   local lane for a week. Keep `EICHLER_QUEUE=pueue` working.
...
* **Receipt readers.** Grepped: none outside `tools/validation/`.
  ...
  `EICHLER_VALIDATION_DIRECTORY`, which the local lane must keep setting —
...
2. **`EICHLER_VALIDATION_DIRECTORY`.** Should the local lane set it to the run
...
3. **Does the drift check earn its cost on every job?** Freezing eichler is
...
| # | What | Result |
|---|---|---|
| 1 | `EICHLER_QUEUE=pandora pnpm validate node tools/check-clock-discipline.test.mjs` | **pass**, exit 0, 4.3 s wall. 11 Node tests. Receipt: `lane: local`, `size_class: small`, `reservation_mib: 1024`, `peak_mib: 132`, `drifted: false` |
...
| 7 | `EICHLER_QUEUE=pandora pnpm validate node --watch` | **exit 1 in 0.3 s** with eichler's own message. Nothing queued, nothing admitted |
| 8 | `EICHLER_QUEUE=direct` and the default `pueue` path | both unchanged |
...
| `pandora/config/examples/eichler.pandora.toml` (the local jobs) | +72 |
| `tools/validate.mjs` (the eichler seam) | +132 / −74, of which 74 is the Pueue path moved verbatim into `throughPueue()` |
