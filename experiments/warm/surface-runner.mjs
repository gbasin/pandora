...
const outputManifest = () => {
  const files = [];
  const visit = path => { for (const name of readdirSync(path)) { const item = join(path, name); const stat = statSync(item); if (stat.isDirectory()) visit(item); else if (stat.isFile()) files.push({ path: relative(source, item), sha256: createHash('sha256').update(readFileSync(item)).digest('hex') }); else throw new Error('unsafe surface build output'); } };
...
const invoke = (extra, report, mode) => spawnSync('pnpm', ['--filter', pkg, 'exec', 'playwright', 'test', ...extra, '--workers=1', '--reporter=' + reporter], { cwd: source, env: { ...process.env, PANDORA_SURFACE_REPORT: report, PANDORA_SURFACE_REPORT_MODE: mode }, stdio: 'inherit' });
...
if (request.action === 'plan') {
  for (const build of [[], ['--mode', 'e2e']]) {
    ...
    if (result.status) process.exit(result.status || 70);
  }
  const inventories = [];
...
writeFileSync(join(results, 'surface-shard.json'), JSON.stringify({ version: 1, plan_id: plan.plan_id, parent_attempt: parentAttempt, source_digest: sourceDigest, app, shard: index, planned_ids: expected, observed_ids: observed.inventory.map(x => x.id), outcomes, exit_code: result.status || 0, detail: observed.status || '' }, null, 2) + '\n');
