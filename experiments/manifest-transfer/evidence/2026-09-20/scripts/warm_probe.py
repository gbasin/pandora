...
import uuid
import sys
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "experiments/warm"))
from snapshot import encode, excluded as is_excluded
...
def main():
    ...
    if uses_source:
        ...
        inventory, manifest = capture(args.repo)
        excluded = [n for n in inventory if is_excluded(n)]
    else:
    ...
    scripts = Path(__file__).resolve().parents[1] / 'experiments/warm'
    ...
    result = run('rsync', '-lpcd', '--from0', '--files-from=-', '--stats',
                 '-e', 'ssh ' + ' '.join(SSH_OPTIONS), *options,
                 str(args.repo if uses_source else output / 'source') + '/', f'{args.host}:{remote}/source/',
                 input=''.join(e['path'] + '\0' for e in manifest), capture_output=True, text=True)
    ...
    verify_script = (scripts / 'snapshot.py').read_text() + '\nverify(Path(' + repr(remote + '/source') + '), json.loads(' + repr(encode(manifest).decode()) + '))\n'
    run(*ssh, 'python3 -', input=verify_script, capture_output=True, text=True)
    if uses_source and capture(args.repo) != (inventory, manifest):
        raise RuntimeError('Source changed during transfer; no worker launched')
    metadata['prelaunch_verify_seconds'] = time.monotonic() - checked
