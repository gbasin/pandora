...
Use a disposable, bootstrapped Acme worktree. The preparation script adds three
untracked files and prints an external routing profile. It refuses to overwrite
existing trial inputs.

```sh
python3 /path/to/pandora/experiments/compiled/prepare.py /path/to/acme-worktree > /tmp/compiled-profile.json
cd /path/to/acme-worktree
...
The Dockerfile compiles the consumer web application with Vite. It copies
...
need to land in Acme. The experiment runs no development server.
...
`apps/web/dist`; previous generations stay in Pandora's local evidence.
